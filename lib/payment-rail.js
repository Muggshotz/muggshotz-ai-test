...
export function feeLineCentsFor(subtotalCents, rail) {
  if (!subtotalCents || subtotalCents <= 0) return 0;
  return Math.ceil(feeRateFor(rail) * subtotalCents + FEE_FIXED_CENTS + HANDLING_FEE_CENTS);
}
export function feeLineDescriptionFor(rail) {
  const pct = (feeRateFor(rail) * 100).toFixed(1);
  return `Payment processing at our processor's standard rate (${pct}% + 30¢) plus a 5¢ handling fee`;
}
