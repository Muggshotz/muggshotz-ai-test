...
export function squareTokenPresent() { return !!ACCESS_TOKEN; }
...
// THE LOCATIONS ON THE ACCOUNT, needing only the token: how the location id
// is found during setup, before it is configured (admin.html shows them).
export async function listLocations() {
  const data = await squareCall("/v2/locations", { method: "GET", tokenOnly: true });
  return (data.locations || []).map((l) => ({ id: l.id, name: l.name || "", status: l.status || "", type: l.type || "" }));
}
...
export async function squareCall(path, { method = "POST", body, tokenOnly = false } = {}) {
  if (!(tokenOnly ? squareTokenPresent() : squareConfigured())) throw new SquareError("Square is not configured (SQUARE_ACCESS_TOKEN / SQUARE_LOCATION_ID).");
...
// A NEW CARD, AND MONEY ON IT. CreateGiftCard registers it (PENDING); an
// ACTIVATE activity gives it its first balance. For a card sold outside
// Square's own orders, Square asks for the buyer's payment instrument ids and
// a reference id in place of an order -- what part two (the physical cards
// sold on the site) will pass; the sandbox minter passes placeholders.
export async function createGiftCard({ type = "DIGITAL", gan } = {}) {
  const body = { idempotency_key: randomUUID(), location_id: LOCATION_ID, gift_card: { type } };
  if (gan) { body.gift_card.gan = gan; body.gift_card.gan_source = "OTHER"; }
  const data = await squareCall("/v2/gift-cards", { body });
  const g = data.gift_card || {};
  if (!g.id) throw new SquareError("Square returned a gift card with no id.");
  return { id: g.id, gan: g.gan || null, state: g.state || null, balanceCents: Number(g.balance_money?.amount ?? 0) };
}
export async function activateGiftCard({ giftCardId, amountCents, referenceId, buyerPaymentInstrumentIds }) {
  const data = await squareCall("/v2/gift-cards/activities", {
    body: {
      idempotency_key: randomUUID(),
      gift_card_activity: {
        ...
        activate_activity_details: {
          ...
          buyer_payment_instrument_ids: buyerPaymentInstrumentIds || undefined
        }
      }
    }
  });
  const a = data.gift_card_activity || {};
  return { activityId: a.id || null, balanceCents: Number(a.gift_card_balance_money?.amount ?? 0), gan: a.gift_card_gan || null };
}
