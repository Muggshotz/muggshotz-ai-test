...
// THE FACES (24 Sep 2026): Bud's gift card art, one character each, no
// amount printed (art/gift-card/web/<face>.jpg). Muggsy joins when his card
// arrives without the "$50" on it.
export const GIFT_FACES = ["needles", "chipper", "sly"];
export function giftFacePath(face) {
  return GIFT_FACES.includes(face) ? `art/gift-card/web/${face}.jpg` : "art/gift-certificate-bill-preview.png";
}
...
export function giftEmailHtml({ code, amountCents, recipientName, message, fromName, siteUrl, forBuyer, face }) {
...
<img src="${siteUrl}/${giftFacePath(face)}" alt="Muggshotz gift certificate" style="width:100%;border-radius:10px">
${head}
