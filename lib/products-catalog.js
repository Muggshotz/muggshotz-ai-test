...
export const PRODUCTS_CATALOG = {
  ...
  "travel-mug-40oz-vacuum": {
    ...
    sizes: {
      // PRICED (2026-08-27, Alyx's call: "just figure to make a $9 profit.
      // That's enough profit for one item... that way the customer gets a
      // really good bargain"), then nudged to $25.95 to keep the house .95
      // ending. Probed live, not estimated:
      //   wholesale        $13.69 flat across all 12 colours
      //   US shipping      $19.39 first item / $17.99 additional
      //   US pre-tax total $45.34
      //   net after Stripe $10.55
      // Alyx asked for the ALL-IN total to read $44.95. It cannot be pinned:
      // that total is US-only and pre-tax — the same cup is $52.14 all-in to
      // Canada ($27.19 shipping) and $46.94 to the EU, and automatic_tax
      // moves it again in every US state. Hitting $44.95 exactly would mean
      // a $25.56 product price, which breaks the very .95 format that was
      // the reason for asking. The .95 lives on the price the customer sees
      // on the tile, so it went there instead.
      // For contrast, travel-mug-40oz-insulated sells at $44.95 against a
      // $39.29 wholesale and nets $3.84 — this tumbler is still cheaper for
      // the customer all-in AND worth nearly three times as much to us.
      "40oz": { price: 25.95 }
    },
