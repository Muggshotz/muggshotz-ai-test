...
// below is a PLACEHOLDER (0), unless otherwise stated — replace each
// with a real number pulled from Printify's dashboard (Products → pick
// blueprint/provider → shipping) or the Shipping Rates API before this
// goes live. Until real numbers are in, checkout is charging $0
...
export const PRODUCTS_CATALOG = {
  ...
  "travel-mug-32oz-gator": {
    ...
    layoutType: "single-image",
    blueprintId: 1235,
    // TODO — NEEDS LOOKUP: I only have the print provider's NAME
    // ("Chill") from web research, not its numeric Printify provider
    // ID. Unlike a missing color variantId, this file has no live
    // fallback for a missing printProviderId — it's required to place
    // a real order. Find it in Printify's dashboard: Catalog → Gator
    // Tumbler, 32oz (blueprint 1235) → the "Chill" provider's ID shows
    // in the page URL or the API response. Left as null on purpose
    // rather than guessing a number, since a wrong provider ID would
    // fail silently or route to the wrong manufacturer.
    printProviderId: null,
    sizes: {
