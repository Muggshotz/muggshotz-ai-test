...
export const PRODUCTS_CATALOG = {
  ...
  "travel-mug-40oz-vacuum": {
    ...
    sizes: {
      // PRICED TO THE TOTAL, NOT TO THE TILE (2026-08-27, Alyx's explicit
      // instruction, twice). The target is the number the CUSTOMER PAYS:
      // $44.95, matching the house 4.95/9.95 ladder. Shipping is $19.39, so
      // the product price is whatever makes that total land:
      //
      //   $44.95 total − $19.39 US shipping = $25.56 product price
      //
      // So this figure is NOT itself meant to sit on the 4.95/9.95 ladder —
      // it is a remainder, and the ladder is satisfied one level up, at
      // checkout. An earlier pass "corrected" it to $25.95 on the theory
      // that the .95 belonged on the tile. It does not, for this product.
      // Do not tidy it back onto the ladder: that breaks the total, which is
      // the number Alyx is actually pricing.
      //
      // Probed live, not estimated:
      //   wholesale        $13.69 flat across all 12 colours
      //   US shipping      $19.39 first item / $17.99 additional
      //   US pre-tax total $44.95
      //   net after Stripe $10.18
      //
      // CAVEAT worth knowing: $44.95 is exact for a US customer before tax
      // only. The same cup totals $52.14 to Canada ($27.19 shipping) and
      // $46.94 to the EU, and automatic_tax moves it again per state.
      // For contrast, travel-mug-40oz-insulated sells at $44.95 against a
      // $39.29 wholesale and nets $3.84 — this tumbler lands the customer at
      // the same total while being worth nearly three times as much to us.
      "40oz": { price: 25.56 }
    },
