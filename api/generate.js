...
export default async function handler(req, res) {
  ...
  try {
    const { image, prompt, theme, deviceId, refImageA, refImageB, currentDesign, size, action, templateMerge, idealise, styleDirective, styleIsDefault, styleRef, styleExaggerate, likeness, shapingRule, bandRatio } = req.body;
...
`;
    ...
    // The masked-outpaint test action (wraparoundOutpaintTest) that lived here
    // painted without charging a token, for the retired wraparound comparison
    // page. Removed 24 Sep 2026 (Alyx): stitching is retired, and a free
    // painting action with no password is a hole. The page went with it.
    ...
    // THE CLASSIC STITCHED WRAPAROUND IS GONE (Alyx, 23 Sep 2026: "it's never
    // the correct fix. It simply doesn't work."). It painted a centre panel and
    // then asked for a left and a right "continuation" and joined them, and the
    // joins drifted. Those continuation calls also rode free -- no credit check,
    // no charge -- so any request carrying the tag painted for nothing. Every
    // wrap is now painted in one pass; every call here is checked and charged.


    let customer = null;
    {
      // (Once guarded by the continuation tag; now every call runs it.)
